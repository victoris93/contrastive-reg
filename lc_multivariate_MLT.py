...
class MLP(nn.Module):
    def __init__(
        ...
        self.decode_target_shared = nn.Sequential(
        ...
        self.target_specific_layers = nn.ModuleList([
            nn.Linear(input_dim_target, 1) for _ in range(input_dim_target)
        ])
        self.init_weights(self.target_specific_layers)
    ...
    def decode_targets(self, embedding):
        shared_representation = self.decode_target_shared(embedding)
        specific_outputs = [layer(shared_representation) for layer in self.target_specific_layers]
        concatenated_output = torch.cat(specific_outputs, dim=1)
    ...
    with tqdm(range(num_epochs), desc="Epochs", leave=False) as pbar:
        for epoch in pbar:
            ...
            for features, targets in train_loader:
                ...
                target_decoding =torch.mean((torch.abs(torch.cat(n_views*[targets], dim=0)- out_target_decoded)/ torch.abs(torch.cat(n_views*[targets], dim=0)+1e-10)))
...
df.to_csv(f"results/multivariate/mtl_1.csv", index=True)
...
embedding_df.to_csv(f"results/multivariate/embedding_mtl_1.csv", index=False)
...
all_losses_file_path = "results/multivariate/loss_mtl_1.csv"
