...
def train(train_dataset, test_dataset, mean, std, model=None, device=device, kernel=cauchy, num_epochs=100, batch_size=32):
    ...
    optimizer_embeddings = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_embeddings, factor=0.1)
    
    optimizer_decoding = optim.Adam(model.parameters(), lr=0.01, weight_decay=weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_decoding, factor=0.1)
    ...
    with tqdm(range(num_epochs), desc="Epochs", leave=False) as pbar:
        for epoch in pbar:
            ...
            for features, targets in train_loader:
                ...
                loss_embeddings = kernel_feature+ kernel_target + joint_embedding
                loss_embeddings.backward()
                optimizer_embeddings.step()
                
              
                
                optimizer_decoding.zero_grad()
                ...
                target_decoding.backward()
                optimizer_decoding.step()
                
                loss = loss_embeddings + target_decoding 
