...
REGION_LABELS_NOT_TO_DEACTIVATE = [b'7Networks_RH_Vis_2', b'7Networks_LH_DorsAttn_Post_1']
...
SELECTED_REGIONS_2 =None# [b'7Networks_RH_Vis_2', b'7Networks_LH_DorsAttn_Post_1']
REGIONS_TO_SHUFFLE = [b'7Networks_RH_Vis_2', b'7Networks_LH_DorsAttn_Post_1']
# %%
...
#                     aug_params[aug] = transform_args # to save later in the metrics df
            ...
            with torch.no_grad():
                ...
                test_dataset = Subset(dataset, test_indices)
                test_feat= test_dataset.dataset.matrices[test_dataset.indices]


                test_features = torch.from_numpy(test_features).to(torch.float32)
                test_t_1 = torch.from_numpy(test_target_1).to(torch.float32)
                test_t_2 = torch.from_numpy(test_target_2).to(torch.float32)
                
                out_feat_test = model.transform_feat(test_features)
                pred_test_target_1 = model.decode_targets_1(out_feat_test)
                pred_test_target_2 = model.decode_targets_2(out_feat_test)
                mae_1 = (test_t_1 - pred_test_target_1).abs().mean() / len(test_t_1)
                mae_2 = (test_t_2 - pred_test_target_2).abs().mean() / len(test_t_2)
                atlas_data = fetch_atlas_schaefer_2018(n_rois=100, yeo_networks=7, resolution_mm=1)
                atlas_labels = atlas_data.labels
                feature_performance_1 = []
                feature_performance_2 = []
                for labels in REGIONS_TO_SHUFFLE : 
                        idx = np.where(atlas_labels == labels)[0][0]
                        shuffling_perf_1 = []
                        shuffling_perf_2 = []
                        for _ in range(n_repeats) :
                            ...
                            #val = mat[idx,:]
                            print("val",val.shape)
                            shuffled_indices = torch.randperm(val.size(1))
                            shuffled_val = val[:,shuffled_indices].squeeze(1)
                            
                            print("shuffled_val", shuffled_val.shape)
                            #print("shuffled_val transpose", shuffled_val.permute(1,0).shape)
                            mat[:, idx, :] = shuffled_val
                            #mat[idx,:]= shuffled_val
                            mat[:, :, idx] = shuffled_val.permute(0,2,1)
                            #mat[:,idx] = shuffled_val.permute(1, 0)
                            mat_np = mat.numpy()
                            mat_vec = sym_matrix_to_vec(mat_np, discard_diagonal=True)
                            mat_t = torch.from_numpy(mat_vec).to(torch.float32)
                            out_shuffled_feat = model.transform_feat(mat_t)
                            shuffled_target_1_decoded = model.decode_target_1(out_shuffled_feat)
                            shuffled_target_2_decoded = model.decode_target_2(out_shuffled_feat)
                            mae_shuffled_1 = (y - shuffled_target_1_decoded).abs().mean() / len(y)
                            mae_shuffled_2 = (z - shuffled_target_2_decoded).abs().mean() / len(z)
                            mae_shuffled_1 = mae_1 - mae_shuffled_1
                            mae_shuffled_2 = mae_2 - mae_shuffled_2
                            shuffling_perf_1.append(mae_shuffled_1)
                            shuffling_perf_2.append(mae_shuffled_2)
                        shuffling_perf_1 = shuffling_perf_1.mean().item()
                        feature_performance_1.append(shuffling_perf_1)
                        shuffling_perf_2 = shuffling_perf_2.mean().item()
                        feature_performance_2.append(shuffling_perf_2)
                        
            self.results = (losses, predictions_1, predictions_2, feature_performance_1, feature_performance_2)      
...
# %%
losses, predictions_1, predictions_2, feature_performance_1, feature_performance_2 = zip(*experiment_results)
...
prediction_mape_1.to_csv(f"results/prediction_mape_1_replacing_ffa_2_100xp.csv", index=False)
prediction_var_1.to_csv(f"results/prediction_var_1_replacing_ffa_2_100xp.csv", index=False)

prediction_mape_2.to_csv(f"results/prediction_mape_2_replacing_ffa_2_100xp.csv", index=False)
prediction_var_2.to_csv(f"results/prediction_var_2_replacing_ffa_2_100xp.csv", index=False)

feature_performance_1_df = pd.DataFrame(feature_performance_1, columns=["train ratio", "experiment", "dataset", "region label", "feature_performance_1"])
feature_performance_2_df = pd.DataFrame(feature_performance_2, columns=["train ratio", "experiment", "dataset", "region label", "feature_performance_2"])

feature_performance_1_df.to_csv(f"results/feature_performance_1.csv", index=False)
feature_performance_2_df.to_csv(f"results/feature_performance_2.csv", index=False)
