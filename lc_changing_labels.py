...
class MatData(Dataset):
    def __init__(self, path_feat, path_targets, target_name, threshold=THRESHOLD, region_label_to_deactivate = REGION_LABELS_TO_DEACTIVATE, region_label_not_to_deactivate = REGION_LABELS_NOT_TO_DEACTIVATE, selected_regions = SELECTED_REGIONS, selected_regions_2 = SELECTED_REGIONS_2):
...
def calculate_mape(predicted, actual):
    return (np.abs(predicted - actual) / np.abs(actual)).mean() * 100
# %% ## Training
def run_experiment_set(label, train, test_size, indices, experiment_count, dataset, augmentations, random_state, device, n_sub):
    ...
    with executor.batch():
        for train_ratio in tqdm(np.linspace(.1, 1., 5)):
            ...
            for experiment in tqdm(range(experiment_count)):
                ...
                job = executor.submit(run_experiment, train, test_size, indices, train_ratio, experiment_size, experiment, dataset, augmentations, random_state, device)
...
for label in atlas_labels:
    SELECTED_REGIONS_2 = [label]
    dataset = MatData(path_feat, path_target, "BentonFaces_total", threshold=THRESHOLD,
    region_label_to_deactivate = REGION_LABELS_TO_DEACTIVATE, 
    region_label_not_to_deactivate = REGION_LABELS_NOT_TO_DEACTIVATE, 
    selected_regions = SELECTED_REGIONS, selected_regions_2 = SELECTED_REGIONS_2)
    n_sub = len(dataset)
    print("n_sub", n_sub)
    indices = np.arange(n_sub)
    ...
    experiment_jobs = run_experiment_set(
        label, train, test_size, indices, experiments, dataset, AUGMENTATION, random_state, None, n_sub
    )
    ...
    prediction_metrics = []
    for k, v in prediction_metrics:
        mape = calculate_mape(v[1], v[0])
        variance_explained = calculate_variance_explained(v[1], v[0])
        prediction_metrics.append(k + (mape, variance_explained))
    ...
    prediction_metrics.to_csv(f"results/tests_ffa_2/{label}.csv", index=True)