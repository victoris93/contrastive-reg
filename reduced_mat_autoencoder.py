...
import sys
from ContModeling.modeling import test_reduced_mat_autoencoder, train_reduced_mat_autoencoder
...
def main(cfg: DictConfig):
    ...
    dataset = MatData(dataset_path, targets, synth_exp, reduced_mat=True, vectorize=True, threshold=0)
    ...
    if multi_gpu:
        ...
        with executor.batch():
            for fold, (train_idx, val_idx) in enumerate(kf.split(train_val_idx)):
                ...
                job=executor.submit(run_train_fold, fold, train_reduced_mat_autoencoder, train_idx, val_idx, train_val_dataset, model_params_dir=model_params_dir, cfg=cfg, random_state=random_state)
