...
class PhenoProj(nn.Module):
    def __init__(
        ...
        self.feat_to_target_embedding = nn.Sequential(
            ...
            nn.Linear(hidden_dim_feat, hidden_dim_target),
...
class TargetAutoEncoder(nn.Module):
    def __init__(
        ...
        self.decode_target = nn.Sequential(
            nn.Linear(hidden_dim_target, hidden_dim_feat),
            nn.BatchNorm1d(hidden_dim_feat),
            nn.ELU(),
            nn.Dropout(p=dropout_rate),
...
class Pretraining(nn.Module):
    def __init__(
        ...
        self.target_e = TargetEncoder(input_dim_target,
                                           hidden_dim_feat,
                                           hidden_dim_target,
                                           dropout_rate)
    ...
    def encode_targets(self, y):
        return self.target_e.encode_targets(y)
...
""" class FeatDecodFromEmbSpace(nn.Module):
    def __init__(
        self,
        input_dim_feat,
        input_dim_target,
        ...
        super(FeatDecodFromEmbSpace, self).__init__()
        ...
        self.init_weights(self.target_e.encode_target)
        ...
        self.decode_feat_from_emb_space = nn.Sequential(
            nn.Linear(hidden_dim_target, hidden_dim_feat)
            nn.BatchNorm1d(hidden_dim_feat),
            nn.ELU(),
            nn.Dropout(p=dropout_rate),
        ...
        self.init_weights(self.decode_feat_from_emb_space)
    ...
    def decode_feat_from_emb_space(self, feat_cont_embedding):
        return self.decode_feat_from_emb_space(feat_cont_embedding)
