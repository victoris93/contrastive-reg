...
class GPC_MLP(nn.Module):
    ...
    def transform_feat(self, x):
        ...
        gpc_features_cpu = gpc_features.detach().cpu()
        gpc_features_np = gpc_features_cpu.numpy()
        gpc_features = torch.from_numpy(sym_matrix_to_vec(gpc_features_np, discard_diagonal=True)).float().to(device)
        x = x.detach().cpu().numpy()
        x_vect = torch.from_numpy(sym_matrix_to_vec(x, discard_diagonal=True)).float().to(device)
        feat = gpc_features + x_vect
        features = self.feat_mlp(feat)
